...
import { addTripPlanToDbService, deleteTripPlanByIdService, getTripPlanByIdService } from '../services/tripServices.js';
...
export const createTripPlanHandler = async (req, res) => {
  try {
    ...
    try {
      ...
      console.error('Failed to parse response text as JSON:', parseError.message);
    ...
    const detailedPlacesPromises = allPlaceNames.map(place => getPlaceDetails(place, getToken(req)));
    ...
    res.json({
      trip_plan_id: savedTripPlan._id,
      ...combinedData
    });
  } catch (error) {
...
export const getTripPlanHandler = async (req, res) => {
  try {
    const {
      id
    } = req.params; // Assuming you pass the planId as a route parameter
...
export const deleteTripPlanHandler = async (req, res) => {
  try {
    const {
      id
    } = req.params; // Extract trip plan ID from URL parameters
