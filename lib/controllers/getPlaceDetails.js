...
const getPlaceDetailsController = async (req, res) => {
  ...
  try {
    ...
    if (places.length > 0) {
      ...
      const workingHours = place.opening_hours ? place.opening_hours.weekday_text : 'No working hours available';
      const imageUrl = place.photos ? `https://maps.googleapis.com/maps/api/place/photo?maxwidth=400&photoreference=${place.photos[0].photo_reference}&key=${process.env.GOOGLE_PLACE_API_KEY}` : null;
    ...
    res.status(500).json({
      error: 'Error fetching place details from Google Places API'
    });
  }
};
